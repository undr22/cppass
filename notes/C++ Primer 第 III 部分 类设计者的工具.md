...
- 改变对象状态的运算符，或与给定类型关系密切的运算符，如递增、递减、关系、解引用运算符等，一般是成员函数
- 具有对称性的运算符可能转换任意一端的运算对象，如算术、相等性、关系、位运算符，一般是非成员函数
...
接上条，对形如 `p->mem` 的表达式来说，`p` 必须是指向类对象的指针或是一个重载了 operator-> 的类的对象，根据 `p` 类型的不同，`p->mem` 分别等价于：
